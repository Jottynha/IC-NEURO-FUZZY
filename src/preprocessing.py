...
def preprocess_dataset(name: str, dataset_id: int, output_root: Path, random_state: int = 42) -> None:
    print(f"\n{'='*60}")
    ...
    # Tratamento de valores faltantes e codificação
    ...
    (dataset_dir / "metadata.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Salvo em: {dataset_dir}")

def main() -> None:
    parser = argparse.ArgumentParser(description="Pré-processa 4 bases de classificação do UCI")
    ...
    datasets = {"adult": 2, "bank_marketing": 222, "heart_disease": 45, "mushroom": 73}
