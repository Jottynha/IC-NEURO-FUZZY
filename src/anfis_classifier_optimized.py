"""
ANFIS simplificado com configuração otimizada baseada no estudo incremental.
Executa 21 seeds e salva resultados em `resultados[2]/anfis_optimized/`.
"""
...
# Configuração otimizada derivada do estudo em resultados[2]/anfis_hyperparameter_study
PARAM_GRID = [
    {
...
def prepare_data(data, seed: int, params: Dict[str, Any]):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Executa ANFIS otimizado")
    ...
    for dataset_name in selected_datasets:
        ...
        if not dataset_path.exists():
            ...
            continue
        print(f"\n[Treinando ANFIS (Otimizado) para {dataset_name}]")
    ...
    write_report(
        title="RESULTADOS - ANFIS (ADAPTIVE NEURO-FUZZY INFERENCE SYSTEM) - OTIMIZADO",
        experiments=experiments,
        output_txt=Path("resultados[2]/anfis_optimized/resultados_anfis_otimizado.txt"),
        output_csv=Path("resultados[2]/anfis_optimized/resultados_anfis_otimizado_melhores.csv"),
        output_all_params_csv=Path("resultados[2]/anfis_optimized/resultados_anfis_otimizado_todos.csv"),
        output_json=Path("resultados[2]/anfis_optimized/resultados_anfis_otimizado_detalhado.json"),
    )
    save_global_summary(experiments, Path("resultados[2]/anfis_optimized/resumo_anfis_otimizado.csv"))
    print("\nResultados do ANFIS (Otimizado) salvos em resultados[2]/anfis_optimized/")
